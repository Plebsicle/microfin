...
export default function () {
  ...
  // Deposit Request
  const depositRes = http.post(`${BASE_URL}/deposit`, JSON.stringify({ accountNumber, amount: 1000 }), { headers: authHeaders, tags: { name: 'deposit' }});
  depositDuration.add(depositRes.timings.duration);
  const depositCheck = check(depositRes, { 'Deposit successful': (r) => r.status === 202 });
...
export function handleSummary(data) {
  ...
  console.log(`      ↳ ${(data.metrics.deposit_success.values.rate * 100).toFixed(2)}% — ✓ ${data.metrics.iterations.values.count - failedRequests} / ✗ ${failedRequests}`);
